...
def cleanUp(rawWords):
  ...
  for i in range(0,len(sumarr)):
    ...
    if len(sumarr[i]) > 1:
      ...
      if sumarr[i][-1] in punct1 or sumarr[i][-1] in punct2: sumarr[i] = sumarr[i][:-1]   # once more to delete double punctuations.
      if sumarr[i][0] in punct1 or sumarr[i][0] in punct2: sumarr[i] = sumarr[i][1:]   # delete punctuation at the start of the word.      
    if len(sumarr[i]) > 2:
  ...
  return [t for t in sumarr if ("'" not in t and t.lower() not in stops)]
...
allWords = cleanUp([open(path+f).read() for f in os.listdir(path)])
